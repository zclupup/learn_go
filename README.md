...
| Lesson 39 | Wire 自动依赖注入 | `lesson39_wire_di/` | ✅ 完成 |
| Lesson 40 | issue_api main 启动逻辑 | `lesson40_main_startup/` | ✅ 完成 |
| Lesson 41 | 临时 | `lesson41_xxx/` | ⬜ 下一课 |

**当前进度：已完成 Lesson 01–40。已掌握 Go 基础语法、分层架构、依赖注入、泛型、RWMutex、slog、jsonparser、JWT、TOML 配置、issue_api 全览、Kratos Gin 嵌套路由、逐层追踪完整请求链路、Wire 自动依赖注入、main 启动流程，能独立阅读 issue_api 任意接口和启动逻辑。**
...
- Wire 不是运行时框架，是**编译时代码生成器**：你写 `wire.go` 声明依赖关系，运行 `wire` 命令，自动生成 `wire_gen.go` 组装代码。
- 核心就两个概念：`wire.Build(...)` 声明 Provider 列表，`wire.NewSet(...)` 把一组 Provider 打包成 ProviderSet。
- Wire 通过函数签名推导依赖关系：`NewUserUseCase(repo *UserRepo)` 需要 `*UserRepo`，Wire 自动找能提供 `*UserRepo` 的 Provider（`NewUserRepo`）。
- 为什么需要 Wire：issue_api 有几十个对象，手写组装代码容易漏参数、改签名要改多处，Wire 自动推导并生成全部组装代码。
- ProviderSet 模式：每个包（server/service/biz/data/account）用 `wire.NewSet(...)` 导出自己的 ProviderSet，wire.go 只需 5 行 `wire.Build(...)` 引用它们。
- 对照 issue_api：`wire.go` 第 22 行 `wire.Build(server.ProviderSet, service.ProviderSet, biz.ProviderSet, data.ProviderSet, account.ProviderSet, newApp)` 生成 80 行的 `wire_gen.go`。
- 生成的代码和你手写的完全一样：`repo := NewXxx()` → `usecase := NewXxx(repo)` → `service := NewXxx(usecase)`，只是对象多、名字长。

#### 💬 本课疑问与答疑
- 问：Wire 和 Spring 的依赖注入有什么区别？
  答：Spring 是运行时通过反射注入，Wire 是编译时生成代码然后编译进二进制。Wire 生成完就退场了，运行时没有任何 Wire 代码在跑。
- 问：`wire_gen.go` 能手动改吗？
  答：不能。文件头写了 `// Code generated by Wire. DO NOT EDIT.`，改了下次跑 `wire` 会被覆盖。要改的是 `wire.go` 或各个 Provider 函数。
- 问：怎么知道 Wire 有没有生成对？
  答：看 `wire_gen.go` 里生成的代码，和你手写 DI 的逻辑应该完全一致。编译通过就说明类型和依赖都对上了。

### Lesson 40 — issue_api main 启动逻辑

- issue_api 启动流程共 8 步，对应 `cmd/issue_api/main.go` 的 128 行代码。
- 第 1 步 `flag.Parse()`：读取 `-conf` 命令行参数，确定配置目录路径（Lesson27 学过的 flag 用法）。
- 第 2 步 `config.Load()` + `c.Scan(&bc)`：读取 yaml 文件，解析到 `conf.Bootstrap` 结构体（Lesson27 学过的 yaml 解析）。
- 第 3 步初始化日志：创建 logger，存入 `tool.GlobalLogger` 全局变量，项目中各处通过 `tool.Log()` 使用（Lesson31 学过的 slog 模式）。
- 第 4 步 `validateRequiredConfig`：校验关键配置是否缺失，缺失直接 panic，遵循"快速失败"原则。
- 第 5 步 `InitOtel`：初始化 OpenTelemetry 链路追踪，defer 中 5 秒超时优雅关闭（Lesson15 学过的 context 超时）。
- 第 6 步 `biz.Init()`：初始化 Redis、雪花 ID、飞书/Jira 配置、按 CPU 核数创建 channel 队列（Lesson14 学过的 channel）。
- 第 7 步 `wireApp()`：Wire 生成的组装函数，创建数据库连接、所有 repo/usecase/service、HTTP server、Cronjob server（Lesson28/39 学过的 DI + Wire）。
- 第 8 步 `app.Run()`：启动 HTTP 服务 + 定时任务，阻塞等待退出信号，收到信号后按 defer 逆序清理资源（Lesson13 学过的 defer）。
- 退出时的 defer 链（后进先出）：cleanup() 关数据库 → otelShutdown() 关链路追踪 → c.Close() 关配置读取器。

#### 💬 本课疑问与答疑
- 问：init() 和 main() 有什么区别？
  答：init() 在 main() 之前自动执行，一个包可以有多个 init()，用于注册 flag、初始化全局状态等不需要 main 传入参数的逻辑。main() 是程序唯一入口。
- 问：为什么 biz.Init() 放在 wireApp() 之前？
  答：biz.Init() 处理的 Redis、雪花 ID、队列等是全局基础设施，不依赖具体的业务对象。wireApp() 才创建数据库连接和业务对象，顺序不能颠倒。
- 问：defer 链的执行顺序是怎样的？
  答：后进先出（栈）。main 里按先后顺序写了 3 个 defer：c.Close() → otelShutdown → cleanup()，执行时逆序：cleanup() → otelShutdown → c.Close()。
